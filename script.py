...
def upload_to_youtube(video_file, title, description):
    try:
        ...
        request = youtube.videos().insert(
            part="snippet,status",
            body={
                "snippet": {"title": title, "description": description, "tags": ["shorts", "riddle", "quiz"], "categoryId": "27"},
                "status": {"privacyStatus": "public", "selfDeclaredMadeForKids": False}
            },
            media_body=MediaFileUpload(video_file, chunksize=-1, resumable=True)
        )
        response = request.execute()
        print(f"🚀 YT Upload Success! ID: {response['id']}")
...
def draw_overlay(text, timer=None, is_answer=False, hint=None):
    ...
    y = 550
    for line in textwrap.wrap(f"ANSWER:\n{text}" if is_answer else text, width=26):
    ...
    if timer and not is_answer:
        ...
        draw.rectangle([50, 1420, 50 + int((timer/5)*980), 1435], fill=(255, 60, 60))
...
def main():
    # 1. Topic Logic (Random if topics.txt empty)
    topic = "something mysterious"
    if os.path.exists('topics.txt'):
        ...
        topic = next((t for t in topics if t not in done), "something mysterious")

    # 2. Gemini Stable URL
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite-preview:generateContent?key={KEYS[0]}"
    prompt = f"Create a mysterious riddle about {topic}. If {topic} is generic, pick a cool random object. Return ONLY JSON: {{'question': '...', 'answer': '...', 'hint': '...', 'bg_keyword': 'mysterious keyword', 'topic_name': '...'}}"
    ...
    try:
        ...
        data = json.loads(raw)
        topic = data.get('topic_name', topic)
    except: return
    ...
    upload_to_youtube("riddle.mp4", f"Riddle of the Day: {topic.title()}! #shorts #riddle", f"Can you solve this {topic} riddle? Comment your answer! #quiz #shorts")
